...
pub fn send (stream: &mut TcpStream, msg: Message) {
    send_to_server(stream, msg);
    receive_from_server(stream);
}

fn send_to_server (stream: &mut TcpStream, msg: Message) {
...
fn receive_from_server (stream: &mut TcpStream) -> String {
